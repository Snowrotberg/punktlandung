...
function pinCursorUrl(color: string): string {
  const safeColor = /^#[0-9a-f]{6}$/i.test(color) ? color : playerColorAt(0);
...
export function GameView({ room, me, isHost, onGuess, onCancelRound, onSkipLocation, onImageReady, onLeave, redesign = false, rankedSyncStatus, pendingUploadCount = 0 }: GameViewProps) {
  ...
  useEffect(() => {
    ...
    // A round is not timed until the image has actually fired onLoad. The
    // image pause deliberately keeps both timestamps null.
    if (room.status !== "guessing" || !room.roundStartedAt || !room.roundEndsAt) return;
  ...
  }, [playCountdownTick, room.currentRound, room.roundEndsAt, room.roundStartedAt, room.status]);
  ...
  const currentPlayerColor = playerColorForId(room.players, isLocalRoom ? selectedLocalPlayer?.id : me?.id);
  ...
  const readyToSubmit = Boolean(guess && room.roundStartedAt !== null && !alreadySubmitted && !currentPlayerTimedOut && targetPlayerId);
  ...
  const mapPanelLayout = fullMap
    ? "fixed bottom-3 right-3 h-[calc(100svh-1.5rem)] w-[calc(100vw-1.5rem)] cursor-[crosshair] sm:absolute sm:bottom-4 sm:right-4 sm:h-[calc(100svh-2rem)] sm:w-[calc(100vw-2rem)] min-[1900px]:bottom-4 min-[1900px]:right-4 min-[1900px]:h-[calc(100svh-2rem)] min-[1900px]:w-[calc(100vw-2rem)]"
    : expanded
      ? "absolute bottom-3 left-3 right-3 h-[min(56svh,470px)] cursor-[crosshair] sm:bottom-4 sm:left-auto sm:right-4 sm:h-[min(56svh,540px)] sm:w-[min(58vw,720px)] min-[1900px]:h-[min(56svh,580px)] min-[1900px]:w-[min(52vw,820px)] min-[2400px]:w-[min(48vw,980px)]"
  ...
  return (
    <main className={`punktlandung-game-shell fixed inset-0 overflow-hidden bg-slate-950 ${redesign ? redesignStyles.redesign : ""}`}>
      ...
      {!chromeSuppressed && !fullMap && (
        <AdContainer
          ...
          label="Anzeige"
          position="absolute"
          className="punktlandung-game-ad pointer-events-auto bottom-3 left-3 z-40 hidden lg:block sm:bottom-4 sm:left-4"
        />
      )}
              ...
              {activePlayerList.map((player) => {
                ...
                return (
                  <button
                    ...
                    className={`punktlandung-map-player-badge relative inline-flex min-h-9 max-w-full items-center justify-center overflow-hidden rounded-md border bg-slate-950/70 px-3 py-1.5 text-center text-xs font-black transition sm:min-h-10 sm:text-sm ${
                      active
                        ? "text-white"
                        : submitted || timedOut
                          ? "text-slate-500"
                    ...
                    style={{
                      "--player-color": playerColorForId(room.players, player.id)
                    } as CSSProperties}
                  >
