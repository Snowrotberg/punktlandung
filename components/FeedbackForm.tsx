...
export function FeedbackForm({ context, onSubmitted, autoFocus = false, className = "" }: FeedbackFormProps) {
  ...
  if (submitState === "sent") {
    return (
      <div className={`punktlandung-feedback-success rounded-md border border-emerald-300/45 bg-emerald-400/10 p-5 ${className}`} role="status">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-300">Damit hilfst du, Punktlandung während der öffentlichen Beta gezielt zu verbessern.</p>
      </div>
    );
  }

  return (
    <form className={`punktlandung-feedback-form grid gap-4 ${className}`} onSubmit={submitFeedback}>
      <div>
        <label htmlFor="feedback-message" className="block text-xs font-black uppercase tracking-[0.2em] text-indigo-300">
          Dein Feedback
        </label>
        <textarea
          id="feedback-message"
          ...
          required
          autoFocus={autoFocus}
          placeholder="Was hat gut funktioniert? Wo gab es Probleme oder was wünschst du dir?"
          className="punktlandung-feedback-field mt-2 min-h-32 w-full resize-y rounded-md px-3.5 py-3 text-base leading-6 outline-none transition"
        />
