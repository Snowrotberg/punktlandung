    ...
    <main className="min-h-dvh bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.12),transparent_35%),#020617] px-3 py-4 text-slate-50 sm:px-6 sm:py-6 lg:px-8">
      <div className="mx-auto grid w-full max-w-[112rem] gap-4">
        <header className="flex flex-wrap items-end justify-between gap-4 rounded-2xl border border-slate-700/80 bg-slate-950/82 px-4 py-4 shadow-2xl backdrop-blur sm:px-6">
          <div>
            ...
            <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-300">
              Referenzansicht der gemeinsamen Produktions-Engine für Startseite und Solo-Auflösung: durchgehende Kamerafahrt, gemeinsame Geometrieregeln, Ergebnis-Pins und DEM-Terrain. Gesondert markierte Laborfälle sind Experimente und ändern den Produktionspfad nicht.
            </p>
          </div>
        ...
        <aside className="grid gap-2 rounded-2xl border border-slate-800 bg-slate-950/72 px-4 py-4 text-xs leading-5 text-slate-400 sm:grid-cols-2 xl:grid-cols-4 sm:px-6">
          <p><strong className="text-slate-200">Distanzklassen:</strong> Produktion kurz unter 90 km, mittel bis 2.500 km, darüber Globe-Fernflug; der 15.000-km-Fall ist als Experiment markiert.</p>
          ...
          <p><strong className="text-slate-200">Terrain:</strong> Solo-Auflösung und Replay verwenden 1,5×; die leichtere Startseitenvorschau 1,0×. Die Labor-Referenz startet mit 1,5×, weitere Stärken sind ausdrücklich Vergleiche.</p>
