...
export async function GET(request: Request, context: { params: Promise<{ gameId: string; roundId: string }> }) {
  if (!rankedModeEnabled()) return rankedUnavailableResponse();
  const { gameId, roundId } = await context.params;
  return createRankedGameHttpApi(request).prompt(request, gameId, roundId);
}
