...
export function assessLandscapeContext(location: GeoLocation): LandscapeContextAssessment | null {
  ...
  const reasons: LandscapeContextRisk[] = [];
  const excluded = isLandscapeContextExcluded(location.id);
  if (excluded) reasons.push("context-excluded");
  if (foregroundSubjectPattern.test(fileName)) reasons.push("foreground-subject");
  if (genericNaturalDetailPattern.test(fileName)) reasons.push("generic-natural-detail");
  const visibleFraction = mobileLandscapeVisibleFraction(location);
  if (visibleFraction !== null && visibleFraction < minimumMobileContextRetention) reasons.push("mobile-extreme-crop");
  ...
  return {
    ...
    status: excluded ? "excluded" : reasons.length > 0 ? "review" : "pass",
    reasons
  };
}
...
export function landscapeContextCatalogFingerprint(assessments: readonly LandscapeContextAssessment[]): string {
  const stableRows = assessments
    .map((entry) => [entry.locationId, entry.imageFile, entry.imageWidth, entry.imageHeight, entry.status, ...entry.reasons].join("|"))
    .sort()
    .join("\n");
  let hash = 0x811c9dc5;
