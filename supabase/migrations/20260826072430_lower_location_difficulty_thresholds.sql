-- Start adaptive classifications earlier while the catalog is still building
-- verified samples: provisional from 10 rounds, stable from 25 rounds.
...
begin
  with observations as (
    select
      ...
      coalesce(round_row.location_snapshot->>'category', game_row.category) as category,
      guess_row.points,
      guess_row.distance_km,
    ...
    select
      ...
      round(avg(
        case
          when category = 'flags' then case when country_correct then 1 else 0 end
          when distance_km < 750 then 1
          else 0
        end
      )::numeric, 6) as success_rate,
    ...
    select
      ...
      case
        when verified_rounds < 10 then 'medium'
        when (
      ...
      case
        when verified_rounds < 10 then 'insufficient'
        when verified_rounds >= 25 then 'stable'
        else 'provisional'
      end as confidence
    from metrics
  ), upserted as (
...
-- Reclassify existing metrics immediately; the existing daily cron schedule
-- continues to invoke the replaced function at 03:15 UTC.
